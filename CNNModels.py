# CNNModels.py defined all CNN models and training methods used in the research
...
def Condensed_AlexNet(inputs, 
    ...
    """ A parametrically reduced version of AlexNet
    ...
    x = tf.nn.conv2d(x, filter=hyperparams["conv3_conv_kernel"], strides=[1, 1, 1, 1], padding="SAME", name="conv4_conv")
    x = add_bias(x)
    x = tf.nn.tanh(x)
    x = tf.nn.max_pool(x, ksize=[1, 2, 2, 1], strides=[1, 1, 1, 1], padding="SAME",name="conv4_maxP")
...
def Condensed_GoogleNet(inputs, 
    ...
    """ A parametrically reduced version of GoogleNet-like CNN
...
def train_predict(X, Y, valX, valY, 
    ...
    def cost_sensitive_loss(y_true, y_pred):
        y_pred = tf.clip_by_value((y_pred*expected_penalty)/tf.reduce_sum(y_pred*expected_penalty, 
                                  axis=-1, keepdims=True), tf.keras.backend.epsilon(), 1-tf.keras.backend.epsilon())
    ...
    xentropy_loss = cost_sensitive_loss(y_batch, pre_thresholded_output)
